...
exports.getAll = function(data, callback) {
  var query = [
    ...
    'WITH other, c, collect(company) as company',
    'MATCH path=(c)-[*]->(m:Message)',
    'RETURN DISTINCT other, collect(m) as messages, c.connectDate as connectDate, company'
  ].join('\n');
...
var processMessages = function(userId, results, callback){
  var companyHelper = function(company){
    console.log('first',company[0])
    console.log('break')
    console.log('break')
    console.log('break')
    console.log('break')
  
    console.log('second',company[0].data)
